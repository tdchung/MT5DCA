...
def run_at_index(mt5_api, symbol, amount, index, price=0, logger=None):
    ...
    # Calculate buy stop entries and TP as percent
    buy_entry_1 = price + (price * DELTA_ENTER_PRICE_PERCENT / 100)
    buy_tp_1 = buy_entry_1 + (buy_entry_1 * TARGET_PROFIT_PERCENT / 100)
    buy_entry_2 = price + (price * (TARGET_PROFIT_PERCENT + DELTA_ENTER_PRICE_PERCENT) / 100)
    buy_tp_2 = buy_entry_2 + (buy_entry_2 * TARGET_PROFIT_PERCENT / 100)
    buy_entry_3 = price + (price * (2 * TARGET_PROFIT_PERCENT + DELTA_ENTER_PRICE_PERCENT) / 100)
    buy_tp_3 = buy_entry_3 + (buy_entry_3 * TARGET_PROFIT_PERCENT / 100)

    # Calculate sell stop entries and TP as percent
    sell_entry_1 = price - (price * DELTA_ENTER_PRICE_PERCENT / 100)
    sell_tp_1 = sell_entry_1 - (sell_entry_1 * TARGET_PROFIT_PERCENT / 100)
    sell_entry_2 = price - (price * (TARGET_PROFIT_PERCENT + DELTA_ENTER_PRICE_PERCENT) / 100)
    sell_tp_2 = sell_entry_2 - (sell_entry_2 * TARGET_PROFIT_PERCENT / 100)
    sell_entry_3 = price - (price * (2 * TARGET_PROFIT_PERCENT + DELTA_ENTER_PRICE_PERCENT) / 100)
    sell_tp_3 = sell_entry_3 - (sell_entry_3 * TARGET_PROFIT_PERCENT / 100)
