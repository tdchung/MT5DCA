...
################################################################################################
CONFIG_FILE = f"config/mt5_config_212034076.json"
...
FIBONACCI_LEVELS = trading_config.get('fibonacci_levels', [1, 1, 2, 2, 3, 3, 3, 5, 5, 5, 5, 8, 8, 8, 8, 8, 8, 13, 13, 13, 13, 13])
TRADE_SYMBOL = trading_config.get('trade_symbol', "XAUUSDc")
DELTA_ENTER_PRICE = trading_config.get('delta_enter_price', 0.4)
...
def run_at_index(mt5_api, symbol, amount, index, price=0, logger=None):
    ...
    try:
        ...
        current_free_margin = get_current_free_margin(mt5_api, logger=logger)
        ...
        if current_free_margin < MIN_FREE_MARGIN:
            if logger:
                logger.error(f"⛔️ Current free margin {current_free_margin} is below minimum required {MIN_FREE_MARGIN}. Stopping further trades.")
            telegramBot.send_message(f"⛔️ Current free margin {current_free_margin} is below minimum required {MIN_FREE_MARGIN}. Stopping further trades.", chat_id=TELEGRAM_CHAT_ID)
            return
