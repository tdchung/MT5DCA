"""
Grid DCA Strategy - Account 159623800
...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, f'grid_dca_159623800_{datetime.now().strftime("%Y%m%d")}.log')
...
def main():
    """
    Main entry point for Grid DCA Strategy (Account ftmo).
    Uses strategy module for complete encapsulation.
    """
    logger = setup_logging()
    logger.info("=" * 60)
    logger.info("Starting Grid DCA Strategy - Account ftmo")
    logger.info("=" * 60)
